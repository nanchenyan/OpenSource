%macro DB_Wind_Industry_SecID(dsn=&DBWIND_dsn.,
...
proc sql; 
	...
	(
		...
		from
				(
					select 
						substring(F5_5014,1,%eval((&IndLevel.+1)*2)) + substring('00000000000000000000000000000000000000',%eval((&IndLevel.+1)*2+1),%eval(38-(&IndLevel.+1)*2)) 
																	as IndCode, 
						F2_1090								as SecID, 
						ob_object_name_1090	as SecName,
						F3_5014								as IndType,
						F9_5014								as StartDate, 
						F10_5014							as EndDate, 
						F11_5014							as IsRecent
					from 
						tb_object_5014 left join tb_object_1090 on F2_5014 = F2_1090
					where 
						substring(F5_5014,1,%eval((&IndLevel.+1)*2)) + substring('00000000000000000000000000000000000000',%eval((&IndLevel.+1)*2+1),%eval(38-(&IndLevel.+1)*2))
...
%DB_Wind_Industry_SecID(	insetDateSecID=inset._AShare_DateSecIDComID,
												insetMap=_Sector_IndMap,
												IndName=,
												IndLevel=2,
												startDate=20120101,
