%macro DB_Wind_Acc_RptFast(	dsn=&DBWIND_dsn.,
															database=&DBWIND_database.,
															uid=&DBWIND_uid.,
															pwd=&DBWIND_pwd.,
															table=TB_OBJECT_1635,
															startDate=,
															endDate=&today.,
															outset=_Acc_RptFast, 
optionon=0);
    ...
    %put &sqlxmsg;
		...
		(
			...
			SELECT
				...
				F1_1635							as ComID,
				.										as ComType,
				F2_1635							as RptDate,									/*报告期*/
				F3_1635							as PubDate,									/*公告日期*/
				CONVERT(char(10), 
				t1.RP_GEN_DATETIME, 112)
														as GenDate,
				F4_1635							as OperIncome,							/*	报表类型*/
				F12_1635						as IsAudit,
				F5_1635							as OperPnL,									/*	 营业收入*/
				F7_1635							as NI,	
				F8_1635							as EPS,											/* 每股收益-基本 */
				F9_1635							as ROEw,										/* 净资产收益率-加权 */
				F10_1635						as TA,
				F11_1635						as Equity			
					
				from
					tb_object_1635 t1 left join TB_OBJECT_1090
					on  F1_1635 = OB_REVISIONS_1090 
				WHERE
					substring(F4_1635,1,4) = '合并报表'
					and F4_1090='A'					/* ChinaA Share						*/
					and F3_1635 is not null		/* DisclosureDate is not null 	*/
					and F1_1635 is not null		/* ComID is not null					*/
					and substring(F2_1635,1,4) >= 2003
																	/* Only stats after year 2003 */
					%if "&startDate"^="" %then %do; 
						and F2_1635>= &startDate.
					%end; 
					%if "&endDate."^="" %then %do; 
						and F2_1635 <= &endDate.
					%end; 
...
AS	
	...
	SELECT
		distinct 
		SecurityID,
		F6_1635 AS IsNewPolicy,
		convert(varchar(8), dateadd(year, -1, cast(F2_1635 as datetime)), 112) AS 'StartDate',
		F2_1635							AS 'EndDate',
		F3_1635							AS 'IssueDate',
		F9_1635							AS 'OperateIncome',		-- 营业收入
		CASE 
			WHEN F10_1635 IS NULL THEN F27_1635 
			ELSE F10_1635 END	AS 'OperateCost',					-- 金融企业为营业支出：F27_1635，其他为营业成本：F10_1635
		F48_1635						AS 'OperateProfit',				-- 营业利润（税前）
		CASE 
			WHEN F55_1635 > 0.1 THEN F48_1635*(1-isnull(F56_1635,0)/F55_1635)
			ELSE F48_1635 END			AS 'OperateProfitAT',	-- 税后净营业利润（NOPAT）
		F61_1635						AS 'NetProfit',						-- 净利润（去除少数股东损益）
		CASE 
			WHEN F55_1635 > 0.1 THEN F55_1635-isnull(F62_1635,0)/(1-isnull(F56_1635,0)/F55_1635)
			ELSE F55_1635 END			AS 'TotalProfit',				-- 税前利润总额（去除税前少数股东损益）
		isnull(F18_1635, F14_1635)		AS 'InterestFee',	-- 利息费用
		EndYear,
		QuaterNum,
		RP_GEN_DATETIME
	INTO #TempProfit
	FROM
		WindDB.wind.dbo.TB_OBJECT_1635,						-- 利润表
		(SELECT distinct FirmID, SecurityID FROM #TempSecurityList) sec,
		#TempReportDate d
	WHERE
		sec.FirmID collate Chinese_PRC_CI_AS = F1_1635 AND F7_1635 = '1' 
																								--是否上市后数据，上市公司需公布上市前3年财务数据
		AND F2_1635 = d.EndDate
		AND F4_1635 = '合并报表' AND F3_1635 IS NOT NULL
		AND ((F3_1635 > @startDate AND F3_1635 <= @endDate) OR (F2_1635 > @startDate AND F2_1635 <= @endDate))
